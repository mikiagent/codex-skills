...
ART_STYLE = (
    ...
    "SOLID COLORED BODY. Fully transparent alpha background, no backdrop or matte. "
...
CHAR_STYLE = (
    ...
    "Fully transparent alpha background, no backdrop, no floor, no shadow "
...
GRID_RULES = (
    ...
    "5. Space between quadrants is fully transparent only."
)
...
GRID_RULES_4X4 = (
    ...
    "5. Character is CENTERED horizontally and vertically within its cell. Fills ~60% of the cell, leaving equal transparent padding on all four sides. "
    "6. Space between cells is fully transparent alpha only."
)
    ...
    if target == "creature":
        ...
        if mode == "player":
            result = (
                ...
                "Front-facing (toward camera), idle standing pose, centered in canvas with lots of transparent padding around. "
                f"{CHAR_STYLE}"
            )
        ...
        if mode == "single":
            result = (
                ...
                "Centered in the canvas with clear transparent padding around it. "
                "Readable silhouette, game-ready shape consistency, true transparent alpha background. "
                f"{ART_STYLE}"
            )
        else:
...
def prepare_sprite_alpha(
    img: Image.Image, threshold: int = 100, edge_threshold: int = 150
) -> Image.Image:
    """Preserve genuine alpha; use legacy chroma cleanup only for fully opaque inputs."""
    rgba = img.convert("RGBA")
    alpha_min, _alpha_max = rgba.getchannel("A").getextrema()
    if alpha_min < 255:
        return rgba
    return remove_bg_magenta(rgba, threshold, edge_threshold)
...
def center_single_sprite(img: Image.Image, size: int, threshold: int, edge_threshold: int) -> Image.Image:
    cleaned = prepare_sprite_alpha(img, threshold, edge_threshold)
...
def cmd_process(args: argparse.Namespace) -> None:
    ...
    if has_custom_grid or args.mode in GRID_SHAPES:
        ...
        cleaned = prepare_sprite_alpha(raw.copy(), args.threshold, args.edge_threshold)
