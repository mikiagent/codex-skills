...
description: "Generate and postprocess general 2D game assets and animation sheets: pixel-art sprites, clean HD map props, creatures, characters, NPCs, spells, projectiles, impacts, props, summons, and transparent PNG/GIF exports. Use when Codex should infer the asset plan from a natural-language request, match a reference or map art style, call built-in `image_gen` for transparent-first raw sheets, and use the local processor only for alpha cleanup, frame extraction, alignment, QC, and transparent exports."
---
...
- Use the script only as a deterministic processor: alpha/chroma cleanup, frame splitting, component filtering, scaling, alignment, QC metadata, transparent sheet export, and GIF export.
...
- Request a truly transparent background (alpha) for every raw sprite or sprite sheet. Require no backdrop, floor plane, cast/contact shadow, glow field, or background pixels.
- Verify alpha numerically before accepting an output. A checkerboard drawn into an opaque PNG is not transparency. For a light neutral baked checkerboard, run `scripts/remove_baked_checkerboard.py`, then re-check transparent corners and subject coverage.
- Preserve valid source alpha as-is during deterministic processing; do not run chroma-key removal over an input that already contains transparent pixels.
- If built-in image generation does not return usable alpha, keep the failed output in `_work/`, regenerate once on a flat non-pink chroma key such as `#00FF00`, and remove it locally before review or runtime use. Never use `#FF00FF`/pink as the default key. Never ship or present a chroma-key intermediate as a finished asset.
...
- fully transparent background (alpha), with no background RGB fringe
- exact sheet shape
...
- `scripts/remove_baked_checkerboard.py`: convert a light neutral checkerboard connected to the canvas border into real alpha without erasing enclosed subject highlights
